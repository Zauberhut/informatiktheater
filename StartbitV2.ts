...
// 6 aktive Spalten liegen auf Bits 7..2
const __FONT_MASK_6 = 0xFC

// Font-Familien (Sans = Arial/Helvetica-ähnlich, Comic = runder/stärker, Serif = "Mignon"-artig, Mono = monospaced)
export enum MatrixFontFamily {
    //% block="Sans (Arial/Helvetica)"
    Sans = 0,
    //% block="Sans Bold"
    SansBold = 1,
    //% block="Comic"
    ...
    //% block="Serif (Mignon)"
    Serif = 3,
    //% block="Mono (feste Breite)"
    Mono = 4
}

let __matrixFontFamily: MatrixFontFamily = MatrixFontFamily.Sans
let __letterSpacing = 1

/**
 * Schriftfamilie für Matrix-Text wählen
 */
//% block="Matrix Schriftfamilie %family"
//% subcategory="Matrix" group="Features" weight=49
export function setMatrixFontFamily(family: MatrixFontFamily) {
    __matrixFontFamily = family
}

/**
 * Zeichenabstand (Pixel) setzen
 */
//% block="Matrix Zeichenabstand %spacing"
//% spacing.min=0 spacing.max=3 spacing.defl=1
//% subcategory="Matrix" group="Features" weight=48
export function setMatrixLetterSpacing(spacing: number = 1) {
    __letterSpacing = Math.max(0, Math.min(3, spacing | 0))
}

// ------------------------------
// Basisfont holen (8 Zeilen à 1 Byte) aus font8x3
// ------------------------------
function __getBaseRows(char: string): number[] {
    const rows: number[] = [0,0,0,0,0,0,0,0]
    const off = char.charCodeAt(0) - 32
    if (off >= 0) {
        for (let i = 0; i < 8; i++) {
            rows[i] = font8x3.getNumber(NumberFormat.UInt8BE, off * 8 + i) & 0xFF
        }
    }
    return rows
}
...
// ------------------------------
// Family-Transformationen
// ------------------------------

// Sans: Identität
function __xf_sans(rows: number[]): number[] {
    const out = new Array<number>(8)
    for (let y = 0; y < 8; y++) out[y] = rows[y] & __FONT_MASK_6
    return out
}

// SansBold: horizontale + leichte vertikale Verbreiterung
function __xf_bold(rows: number[]): number[] {
    const out = new Array<number>(8)
    for (let y = 0; y < 8; y++) {
        const r  = rows[y] & __FONT_MASK_6
        const l1 = (r << 1) & __FONT_MASK_6
        const r1 = (r >> 1) & __FONT_MASK_6
        const up = y > 0 ? (rows[y - 1] & __FONT_MASK_6) : 0
        const dn = y < 7 ? (rows[y + 1] & __FONT_MASK_6) : 0
        out[y] = (r | l1 | r1 | (up & 0xF8) | (dn & 0xF8)) & __FONT_MASK_6
    }
    return out
}

// Comic: runder, spielerischer Look (Dilatation + leichte Schrägung)
function __xf_comic(rows: number[]): number[] {
    const tmp = __xf_bold(rows) // zuerst etwas verdicken
    const out = new Array<number>(8)
    for (let y = 0; y < 8; y++) {
        // sanfte Schrägstellung nach rechts für untere Zeilen
        const shear = (y >= 5) ? 2 : (y >= 2 ? 1 : 0)
        out[y] = (tmp[y] >> shear) & __FONT_MASK_6
    }
    return out
}

// Serif: kleine Serifen an Ober-/Unterkante der aktiven Breite
function __xf_serif(rows: number[]): number[] {
    // aktive Spaltenbreite ermitteln
    const rmask = rows.map(r => r & __FONT_MASK_6)
    let left = -1, right = -1
    for (let c = 7; c >= 2; c--) {
        let any = false; for (let y = 0; y < 8; y++) if (rmask[y] & (1 << c)) { any = true; break }
        if (any) { left = c; break }
    }
    for (let c = 2; c <= 7; c++) {
        let any = false; for (let y = 0; y < 8; y++) if (rmask[y] & (1 << c)) { any = true; break }
        if (any) { right = c; break }
    }
    const out = rmask.slice()
    if (left !== -1 && right !== -1) {
        // dezente Serifen an Top/Bottom: kurze Querstriche an Rändern
        const topBits    = ((1 << left) | (1 << Math.max(2, left - 1)) | (1 << Math.min(7, left + 1))) & __FONT_MASK_6
        const bottomBits = ((1 << right) | (1 << Math.max(2, right - 1)) | (1 << Math.min(7, right + 1))) & __FONT_MASK_6
        out[0]  = (out[0]  | topBits)    & __FONT_MASK_6
        out[7]  = (out[7]  | bottomBits) & __FONT_MASK_6
        // leichte Stammverstärkung
        for (let y = 1; y <= 6; y++) {
            const colL = (out[y] & (1 << left)) ? 1 : 0
            const colR = (out[y] & (1 << right)) ? 1 : 0
            if (colL) out[y] |= (1 << Math.max(2, left - 1))
            if (colR) out[y] |= (1 << Math.min(7, right + 1))
            out[y] &= __FONT_MASK_6
        }
    }
    return out
...
// Mono: identisch zu Sans bei Glyph-Form, aber feste Laufweite (Breite = 6)
function __xf_mono(rows: number[]): number[] {
    return __xf_sans(rows)
}

// ------------------------------
// Variable Breite + Links-Bündig Packen (Bit7..)
// ------------------------------
function __packVarWidth(rows: number[]): { rows: number[], width: number, left: number, right: number } {
    let left = -1, right = -1
    for (let c = 7; c >= 2; c--) {
        let any = false; for (let y = 0; y < 8; y++) if (rows[y] & (1 << c)) { any = true; break }
        if (any) { left = c; break }
    }
    for (let c = 2; c <= 7; c++) {
        let any = false; for (let y = 0; y < 8; y++) if (rows[y] & (1 << c)) { any = true; break }
    ...
    if (left === -1 || right === -1) {
        return { rows: [0,0,0,0,0,0,0,0], width: 0, left: -1, right: -1 }
    }
    const width = left - right + 1
    let mask = 0
    for (let c = right; c <= left; c++) mask |= (1 << c)
    const shift = 7 - left
    const out = new Array<number>(8)
    for (let y = 0; y < 8; y++) out[y] = ((rows[y] & mask) << shift) & __FONT_MASK_6
    return { rows: out, width: Math.min(6, Math.max(0, width)), left, right }
}

// ------------------------------
// Familien-Glyph holen (Form + var. Breite)
// ------------------------------
function __getGlyphForFamilyVarWidth(ch: string): { rows: number[], width: number } {
    const base = __getBaseRows(ch)
    let styled: number[]
    switch (__matrixFontFamily) {
        case MatrixFontFamily.SansBold: styled = __xf_bold(base); break
        case MatrixFontFamily.Comic:    styled = __xf_comic(base); break
        case MatrixFontFamily.Serif:    styled = __xf_serif(base); break
        case MatrixFontFamily.Mono:     styled = __xf_mono(base); break
        default:                        styled = __xf_sans(base); break
    }
    const packed = __packVarWidth(styled)
    if (__matrixFontFamily === MatrixFontFamily.Mono) {
        // Monospaced: feste Breite 6, linksbündig schon gepackt
        return { rows: packed.rows, width: 6 }
    }
    return { rows: packed.rows, width: packed.width }
}

// ------------------------------
// Zeichen zeichnen (vertikal zentriert, mit Clipping)
// ------------------------------
function __drawGlyphVcenteredClipped(matrix: Matrix, rows: number[], x: number, width: number, colour: number): void {
    if (width <= 0) return
    const yOffset = (matrix.Height - 8) >> 1
    for (let col = 0; col < width; col++) {
        const xcol = x + col
        if (xcol < 0 || xcol >= matrix.Width) continue
        const bitMask = (0x80 >> col)
        for (let y = 0; y < 8; y++) {
            if (rows[y] & bitMask) matrix.setPixel(xcol, y + yOffset, colour)
        }
    }
}

// ==============================
// Methoden in Matrix-Klasse
// ==============================

/**
 * Zeige Text (variable Breite je nach Schriftfamilie)
 */
//% block="%matrix Text (var. Breite): %text | Position: %x_offset | Farbe: %colour"
//% subcategory="Matrix" group="Features"
//% x_offset.defl=0 x_offset.min=-128 x_offset.max=128
//% colour.shadow=neopixel_colors
export function __matrix_showTextVarWidth(matrix: Matrix, text: string, x_offset: number, colour: number): void {
    let x = x_offset | 0
    const spacing = __letterSpacing
    for (let i = 0; i < text.length; i++) {
        const g = __getGlyphForFamilyVarWidth(text.charAt(i))
        const adv = (__matrixFontFamily === MatrixFontFamily.Mono) ? (6 + spacing) : ((g.width > 0 ? g.width : 3) + spacing)
        __drawGlyphVcenteredClipped(matrix, g.rows, x, Math.max(1, g.width), colour)
        x += adv
    }
    matrix.show()
}
...
/**
 * Scrolle Text (variable Breite)
 */
//% block="%matrix Scroll (var. Breite): %text | Speed (1-200) %speed | Farbe %colour"
//% subcategory="Matrix" group="Features"
//% speed.min=1 speed.max=200 speed.defl=50
//% colour.shadow=neopixel_colors
export function __matrix_scrollTextVarWidth(matrix: Matrix, text: string, speed: number, colour: number): void {
    const spacing = __letterSpacing
    const glyphs: { rows: number[], width: number }[] = []
    ...
    for (let i = 0; i < text.length; i++) {
        ...
        glyphs.push(g)
        offsets.push(total)
        const adv = (__matrixFontFamily === MatrixFontFamily.Mono) ? (6 + (i < text.length - 1 ? spacing : 0))
            : ((g.width > 0 ? g.width : 3) + (i < text.length - 1 ? spacing : 0))
        total += adv
    }
    const startX = matrix.Width
    const endX = -total
    for (let X = startX; X > endX; X--) {
        matrix.clear()
        for (let i = 0; i < glyphs.length; i++) {
            const pos = X + offsets[i]
            const g = glyphs[i]
            const drawWidth = Math.max(1, g.width)
            if (pos < matrix.Width && pos + drawWidth > 0) {
                __drawGlyphVcenteredClipped(matrix, g.rows, pos, drawWidth, colour)
            }
        }
        matrix.show()
        basic.pause(Math.idiv(1000, Math.max(1, speed)))
    }
}

// ==============================
// Komfort-Methoden direkt in Matrix exposed (Wrapper für Blocks)
// ==============================

/**
 * Matrix: Text (var. Breite) anzeigen
 */
//% block="%matrix Text: %text | x %x | Farbe %colour"
//% subcategory="Matrix" group="Features" weight=47
//% x.defl=0 x.min=-128 x.max=128
//% colour.shadow=neopixel_colors
//% inlineInputMode=inline
Matrix.prototype.showTextVarWidth = function (this: Matrix, text: string, x: number, colour: number): void {
    __matrix_showTextVarWidth(this, text, x, colour)
}

/**
 * Matrix: Text (var. Breite) scrollen
 */
//% block="%matrix Scroll: %text | Speed %speed | Farbe %colour"
//% subcategory="Matrix" group="Features" weight=46
//% speed.min=1 speed.max=200 speed.defl=50
//% colour.shadow=neopixel_colors
//% inlineInputMode=inline
Matrix.prototype.scrollTextVarWidth = function (this: Matrix, text: string, speed: number, colour: number): void {
    __matrix_scrollTextVarWidth(this, text, speed, colour)
}
/**    
* ***********************************
*/
