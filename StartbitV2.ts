...
namespace informatiktheater {
    ...
    // ==============================
    // NEUE FONTS (variable Breite) + Steuerung
    // ==============================

    // 6 aktive Spalten liegen auf Bits 7..2
    const __FONT_MASK_6 = 0xFC

    // Font-Familien (Sans ~ Arial/Helvetica, Comic, Serif ~ Mignon, Mono)
    ...
    // Basisfont (8 Reihen à 1 Byte) aus font8x3 holen
    ...
    // Comic: runder Look (erst leicht verdicken, dann sanft schrägen)
    function __xf_comic(rows: number[]): number[] {
        const tmp = __xf_bold(rows)
        const out = new Array<number>(8)
        for (let y = 0; y < 8; y++) {
    ...
    // Serif: kleine Serifen an Ober-/Unterkante
    ...
    // Mono: gleiche Form wie Sans, aber später feste Breite
    function __xf_mono(rows: number[]): number[] {
        return __xf_sans(rows)
    }

    // var-Breite packen (links bündig auf Bit7, Breite 0..6)
    function __packVarWidth(rows: number[]): { rows: number[], width: number } {
        let left = -1, right = -1
        for (let c = 7; c >= 2; c--) {
        ...
        if (left === -1 || right === -1) return { rows: [0,0,0,0,0,0,0,0], width: 0 }
        ...
        return { rows: out, width: Math.min(6, Math.max(0, width)) }
    }

    // Familien-Glyph var-Breite
    ...
    // Zeichnen einer Glyph (vertikal zentriert, Clipping über setPixel)
    ...
    export class Matrix {
         ...
         * Zig-Zag Mapping intern berücksichtigt.
         */
        ...
        /**
         * Zeige Text (fixe 6x8 Pixel Schrift).
         */
        //% blockId="Matrix_text" block="%matrix Text 32x8: %text|Position: %x_offset|Farbe: %colour"
        //% subcategory="Matrix" 
        //% group="Features"
        //% colour.shadow=neopixel_colors
        //% x_offset.defl=0 x_offset.min=0 x_offset.max=64
        ...
        /**
         * Scrolle Text (fixe 6x8 Pixel Schrift).
         */
         ...
         */
        //% blockId="Matrix_text_var" block="%matrix Text (var. Breite): %text | Position: %x | Farbe: %colour"
        //% subcategory="Matrix" 
        //% group="Features"
        //% colour.shadow=neopixel_colors
        //% x.defl=0 x.min=-128 x.max=128
        showTextVarWidth(text: string, x: number, colour: number): void {
            let xpos = x | 0
            ...
            for (let i = 0; i < text.length; i++) {
                ...
                __drawGlyphVcenteredClipped(this, g.rows, xpos, Math.max(1, g.width), colour)
                xpos += adv
            }
            this.show()
        }
        ...
        /**
         * Scrolle Text (variable Breite je nach Schriftfamilie).
         */
        //% blockId="Matrix_scrollText_var" block="%matrix Scroll (var. Breite): %text | Speed %speed | Farbe %colour"
        //% subcategory="Matrix"
        //% group="Features"
        //% colour.shadow=neopixel_colors
        //% speed.min=1 speed.max=200 speed.defl=50
        scrollTextVarWidth(text: string, speed: number, colour: number): void {
            const spacing = __letterSpacing
            ...
            for (let X = startX; X > endX; X--) {
                ...
                for (let i = 0; i < glyphs.length; i++) {
                    ...
                    if (pos < this.Width && pos + drawWidth > 0) {
                        __drawGlyphVcenteredClipped(this, g.rows, pos, drawWidth, colour)
                    }
                }
                this.strip.show()
    ...
    function getLettermap(char: string): number[] {
        ...
        let offset = char.charCodeAt(0) - 32;
        if (offset >= 0) {
            for (let i = 0; i < 8; i++) {
    ...
    */
    ...
    // MP3 Player stubs (auskommentiert im Original)
}
